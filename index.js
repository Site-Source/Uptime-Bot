        ...
        if (config.Servers)
        {
            ...
            if (newPresence.status == PresenceUpdateStatus.Online)
            {
                newPresence.guild.channels.cache.get(config.Channels).send({
                    ...
                    .setTitle(`📡 ︱ Обратно онлайн`)
                    .setDescription(`Вече **${newPresence.user.tag}** се върна онлайн! Всичко е нормализилно `)
                    
                    .setColor(`GREEN`)
                    .setThumbnail(newPresence.user.avatarURL({ format: "png", size: 1024 }))
                    .setFooter(`Ryster | Development | Alert`, newPresence.user.avatarURL({ format: "png", size: 1024 }))
                    .setTimestamp()]
                });
            }
            ...
            else if (newPresence.status == PresenceUpdateStatus.Offline || newPresence.status == PresenceUpdateStatus.Invisible)
            {
                newPresence.guild.channels.cache.get(config.Channels).send({
                    embeds: [new MessageEmbed()
                    .setTitle(`📡 ︱Прекъсване на връзката`)
                    .setDescription(`За съжаление **${newPresence.user.tag}** офлайн! Бе открит проблем с връзката в **NODE01**`)
                    .setColor(`RED`)
                    .setThumbnail(newPresence.user.avatarURL({ format: "png", size: 1024 }))
                    .setFooter(`Ryster | Development | Alert`, newPresence.user.avatarURL({ format: "png", size: 1024 }))
                    .setTimestamp()]
